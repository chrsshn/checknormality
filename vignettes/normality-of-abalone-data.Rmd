...
``` {r}
dat_for_lm <- dat %>% 
  select (Rings, Type, Diameter, Height, LongestShell, WholeWeight) %>%
...
#### the beta coefficients in our model are linear with respect to the outcome
(see model above)
...
```
Visually we would be a little suspect of the normality of the residuals, but the Shapiro-Wilk Test assures that the residuals can be considered normal
